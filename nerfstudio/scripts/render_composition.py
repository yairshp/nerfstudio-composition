...
def _render_trajectory_video(
    ...
    fg_cameras: Optional[Cameras] = None,
    fg_crop_data: Optional[CropData] = None,
) -> None:
    ...
    if fg_cameras is not None:
        fg_cameras.rescale_output_resolution(rendered_resolution_scaling_factor)
        fg_cameras = fg_cameras.to(pipeline.device)
    ...
    with ExitStack() as stack:
        ...
        with progress:
            for camera_idx in progress.track(range(cameras.size), description=""):
                ...
                #! fg crop data
                fg_aabb_box = None
                if fg_crop_data is not None:
                    bounding_box_min = fg_crop_data.center - fg_crop_data.scale / 2.0
                    bounding_box_max = fg_crop_data.center + fg_crop_data.scale / 2.0
                    fg_aabb_box = SceneBox(torch.stack([bounding_box_min, bounding_box_max]).to(pipeline.device))
                if fg_cameras is not None:
                    fg_camera_ray_bundle = fg_cameras.generate_rays(camera_indices=camera_idx, aabb_box=fg_aabb_box)
                else:
                ...
                if crop_data is not None:
                    with renderers.background_color_override_context(
                        ...
                        outputs = pipeline.model.get_outputs_for_camera_ray_bundle(camera_ray_bundle, fg_pipeline=fg_pipeline)
                else:
                    with torch.no_grad():
                        outputs = pipeline.model.get_outputs_for_camera_ray_bundle(camera_ray_bundle, fg_pipeline=fg_pipeline, fg_camera_ray_bundle=fg_camera_ray_bundle)
...
class RenderCameraPath(BaseRender):
    ...
    def main(self) -> None:
        ...
        if self.load_fg_config is not None:
            _, fg_pipeline, _, _ = eval_setup(
                self.load_fg_config,
                eval_num_rays_per_chunk=self.eval_num_rays_per_chunk,
                test_mode="inference",
            ...
            # fg_camera_path = get_path_from_json(camera_data)
            # fg_camera_path = get_path_from_json(fg_camera_data)
        else:
            fg_pipeline = None
            fg_camera_path = None
            fg_crop_data = None
        ...
        crop_data = get_crop_from_json(camera_data)
        camera_path = get_path_from_json(camera_data)

        if self.load_fg_config is not None:
            fg_camera_path = get_path_from_json(camera_data, do_transformations=True)
